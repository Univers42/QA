...
def _interactive_add() -> dict:
    ...
    # Title
    title = typer.prompt("  Title (what should happen)")
    ...
    if test_type == "http":
        ...
        if add_headers:
            headers_raw = typer.prompt("  Headers (JSON)")
            try:
        ...
        expected_output = typer.prompt("  Expected output (empty to skip)", default="")
    ...
    # Optional metadata
    author = typer.prompt("  Author (your 42 login, empty to skip)", default="")
    if author.strip():
        data["author"] = author

    tags_raw = typer.prompt("  Tags (comma-separated, empty to skip)", default="")
    if tags_raw.strip():
...
def add_test(
    ...
    title: str | None = typer.Option(None, "--title", "-t", help="Test title"),
    domain: str | None = typer.Option(None, "--domain", "-d", help="Domain"),
    priority: str | None = typer.Option(None, "--priority", "-p", help="Priority (P0-P3)"),
    test_type: str | None = typer.Option(None, "--type", help="Test type (http/bash/manual)"),
    url: str | None = typer.Option(None, "--url", help="HTTP URL"),
    method: str | None = typer.Option(None, "--method", "-m", help="HTTP method"),
    expected_status: int | None = typer.Option(
        None, "--expected-status", help="Expected HTTP status"
    ),
    expected_body: str | None = typer.Option(
        None, "--expected-body", help="Expected body contains (comma-separated)"
    ),
    script: str | None = typer.Option(None, "--script", help="Bash script to run"),
    author: str | None = typer.Option(None, "--author", help="Your 42 login"),
):
    """Create a new test definition (interactive or --quick mode)."""
    try:
        if quick:
            ...
            if not all([test_id, title, domain, priority]):
                ...
                console.print(
                    "  [dim]Example: pqa test add --quick --id AUTH-004 "
